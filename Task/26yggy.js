...
if ($.isNode()) {
   if (process.env.yggybody && process.env.yggybody.indexOf('\n') > -1) {
   yggybody = process.env.yggybody.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   yggybody = process.env.yggybody.split()
  };
  Object.keys(yggybody).forEach((item) => {
        if (yggybody[item]) {
          yggybodyArr.push(yggybody[item])
        }
    });
  if (process.env.yggyhd && process.env.yggyhd.indexOf('\n') > -1) {
   yggyhd = process.env.yggyhd.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   yggyhd = process.env.yggyhd.split()
  };
  Object.keys(yggyhd).forEach((item) => {
        if (yggyhd[item]) {
          yggyhdArr.push(yggyhd[item])
        }
    });
  if (process.env.yggyurl && process.env.yggyurl.indexOf('\n') > -1) {
   yggyurl = process.env.yggyurl.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   yggyurl = process.env.yggyurl.split()
  };
  Object.keys(yggyurl).forEach((item) => {
        if (yggyurl[item]) {
          yggyurlArr.push(yggyurl[item])
        }
    });
