...
if ($.isNode()) {
  if (process.env.YQZHCK && process.env.YQZHCK.indexOf('#') > -1) {
  yqzhck = process.env.YQZHCK.split('#');
  console.log(`您选择的是用"#"隔开\n`)
  }
  else if (process.env.YQZHCK && process.env.YQZHCK.indexOf('\n') > -1) {
   yqzhck = process.env.YQZHCK.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   yqzhck = process.env.YQZHCK.split()
  };
  Object.keys(yqzhck).forEach((item) => {
        if (yqzhck[item]) {
          yqzh.push(JSON.parse(yqzhck[item]))
        }
    });
}
